...
function mount(){
  ...
  async function follow(id:string){for(let attempt=0;attempt<30;attempt++){const payload=await api<{jobs:HostQueueItem[]}>('/api/mobile/host/jobs');const item=payload.jobs.find(entry=>entry.job.id===id);history.replaceChildren(...payload.jobs.slice(0,8).map(entry=>{const row=document.createElement('div');row.className='munin-host-row';const name=document.createElement('strong');name.textContent=entry.job.type;const state=document.createElement('span');state.textContent=entry.status;row.append(name,state);return row}));if(item&&item.status!=='queued'&&item.status!=='running'){status.textContent=item.status==='completed'?(item.result?.summary??'Concluído.'):`${item.status}: ${item.result?.summary??'verifique a evidência'}`;return}await new Promise(resolveDelay=>setTimeout(resolveDelay,1000))}status.textContent='A execução continua no PC; consulte o histórico.'}
}
