...
function mount(){
  ...
  async function enqueue(type:HostJobType){
    ...
    try{const item=await api<HostQueueItem>('/api/mobile/host/jobs',{method:'POST',body:JSON.stringify({type})});status.textContent=`Executando ${labels[type]}…`;await follow(item.job.id)}
  ...
  async function follow(id:string){for(let attempt=0;attempt<45;attempt++){try{const payload=await api<{jobs:HostQueueItem[]}>('/api/mobile/host/jobs');const item=payload.jobs.find(entry=>entry.job.id===id);history.replaceChildren(...payload.jobs.slice(0,8).map(entry=>{const row=document.createElement('div');row.className='munin-host-row';const name=document.createElement('strong');name.textContent=entry.job.type;const state=document.createElement('span');state.textContent=entry.status;row.append(name,state);return row}));if(item&&item.status!=='queued'&&item.status!=='running'){status.textContent=item.status==='completed'?(item.result?.summary??'Concluído.'):`${item.status}: ${item.result?.summary??'verifique a evidência'}`;window.setTimeout(()=>panel.classList.remove('open'),1800);return}}catch{status.textContent='PC reiniciando · reconectando…'}await new Promise(resolveDelay=>setTimeout(resolveDelay,1000))}status.textContent='O PC ainda está reiniciando. O painel continuará disponível para nova consulta.'}
}
