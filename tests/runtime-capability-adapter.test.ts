...
// NOTE: retained full suite semantics; expectations reflect the governed capability seam.

test('runtime capability adapter stays disabled by default when explicitly false',async()=>{
 const adapter=new RuntimeCapabilityAdapter(new ExecutionEngine(),{enabled:false});
 assert.equal(adapter.enabled,false);
});

test('runtime capability adapter exposes governed capabilities only when enabled',async()=>{
...
test('disabled adapter fails closed before capability execution',async()=>{
 const adapter=new RuntimeCapabilityAdapter(new ExecutionEngine(),{enabled:false});
 await assert.rejects(()=>adapter.browser({action:'health'}));
});
...
test('enabled adapter executes autonomous loop through capability seam',async()=>{
 const adapter=new RuntimeCapabilityAdapter(new ExecutionEngine(),{enabled:true});
 const result=await adapter.autonomousLoop({objective:'verify capability seam'});
 assert.equal(result.capability,'engineering.autonomous-loop');
 assert.deepEqual(result.trace.map(event=>event.phase),['before','execute']);
});

test('enabled adapter executes engineering mission with injectable runtime',async()=>{
 const adapter=new RuntimeCapabilityAdapter(new ExecutionEngine(),{enabled:true});
 const result=await adapter.engineeringMission({objective:'verify mission seam'});
...
test('local video stays opt-in and never auto-downloads models',async()=>{
 ...
 try{
  const adapter=new RuntimeCapabilityAdapter(new ExecutionEngine(),{enabled:true});
  const result=await adapter.localVideo({action:'health'});
  assert.equal(result.capability,'media.local-video');
