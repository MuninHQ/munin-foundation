...
test('durable orchestration trace store survives new instances and redacts secrets',async()=>{const dir=await mkdtemp(path.join(os.tmpdir(),'munin-traces-'));const file=path.join(dir,'traces.jsonl');try{await new OrchestrationTraceStore(file).append(trace({attempts:[{providerId:'x',ok:false,error:'token=super-secret-value'}]}));const items=await new OrchestrationTraceStore(file).list();assert.equal(items.length,1);assert.match(items[0].attempts[0].error??'',/\[REDACTED\]/);assert.doesNotMatch(items[0].attempts[0].error??'',/super-secret-value/)}finally{await rm(dir,{recursive:true,force:true})}});
