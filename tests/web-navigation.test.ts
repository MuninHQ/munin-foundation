...
test('standalone web pages do not point at missing local HTML/JS/CSS targets',async()=>{
 ...
 for(const file of files){const html=await readFile(join(web,file),'utf8');for(const target of localTargets(html)){if(!(await targetExists(target)))missing.push(`${file} -> ${target}`)}}
