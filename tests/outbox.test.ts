...
async function withLease(root: string) {
  ...
  const acquired = await leases.acquire('plan:1', 'worker-a', 60_000);
...
test('enqueues idempotently before dispatch', async () => {
  ...
  try {
    ...
    const request = {
      operation: 'publish', resourceId: 'doc-1', payload: { title: 'A' },
      leaseKey: 'plan:1', workerId: 'worker-a', fencingVersion: lease.version,
    };
    const first = await outbox.enqueue('recording', request);
    const second = await outbox.enqueue('recording', request);
...
test('dispatches and marks an entry applied', async () => {
  ...
  try {
    ...
    await outbox.enqueue(adapter.id, {
      operation: 'publish', resourceId: 'doc-1', payload: { title: 'A' },
      leaseKey: 'plan:1', workerId: 'worker-a', fencingVersion: lease.version,
    });
    const summary = await outbox.dispatch([adapter]);
    const [entry] = await outbox.list();
    assert.deepEqual(summary, { attempted: 1, applied: 1, failed: 0, skipped: 0 });
...
test('retries failed entries without duplicating applied effects', async () => {
  ...
  try {
    ...
    assert.equal((await outbox.dispatch([adapter])).failed, 1);
    adapter.fail = false;
    assert.equal((await outbox.dispatch([adapter])).applied, 1);
    assert.equal(adapter.calls, 2);
    assert.equal((await outbox.list())[0].status, 'applied');
    assert.equal((await outbox.dispatch([adapter])).attempted, 0);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test('recovers stale dispatch claims', async () => {
  ...
  try {
    const lease = await withLease(root);
    const outbox = new TransactionalOutbox(root);
    const entry = await outbox.enqueue('missing', {
    ...
    entries[0].status = 'dispatching';
    entries[0].claimedAt = new Date(1_000).toISOString();
    await fs.writeFile(file, JSON.stringify(entries, null, 2) + '\n');
    assert.equal(await outbox.recoverStaleDispatches(100, 1_101), 1);
    assert.equal((await outbox.list()).find(item => item.id === entry.id)?.status, 'pending');
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
