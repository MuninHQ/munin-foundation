...
test('Career submission is RED and carries exact typed effect',async()=>{const {dir,runtime}=await fixture();try{const result=await planCareerSubmission(empty(),job,{runtime,now:new Date('2026-09-10T04:00:00Z')});assert.equal(result.sentinel.band,'RED');assert.equal(result.sentinel.disposition,'needs_approval');assert.ok(result.approvalId);assert.equal(result.sentinel.request.effect?.type,'career.submit');assert.equal(result.sentinel.request.effect?.resourceId,'job-1');}finally{await rm(dir,{recursive:true,force:true});}});

test('Video publication is RED and carries exact typed effect',async()=>{const {dir,runtime}=await fixture();try{const result=await planVideoPublication(empty(),'youtube','Wirecard documentary draft',{runtime,now:new Date('2026-09-10T04:00:00Z')});assert.equal(result.sentinel.band,'RED');assert.equal(result.sentinel.disposition,'needs_approval');assert.ok(result.approvalId);assert.equal(result.sentinel.request.effect?.type,'video.publish');assert.equal(result.sentinel.request.effect?.resourceId,'youtube');}finally{await rm(dir,{recursive:true,force:true});}});