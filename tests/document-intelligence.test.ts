...
test('uses Docling outputs when local CLI is available',async()=>{
 ...
 const execFileImpl:any=async(file:string,args:string[])=>{if(file==='where.exe'||file==='which')return {stdout:'/usr/local/bin/docling\n',stderr:''};await writeFile(path.join(out,'sample.md'),'# Structured','utf8');await writeFile(path.join(out,'sample.json'),JSON.stringify({name:'sample'}),'utf8');assert.ok(args.includes('--abort-on-error'));return {stdout:'',stderr:''}};
 const result=await ingestDocument(source,out,{execFileImpl});assert.equal(result.engine,'docling');assert.equal((result.json as any).name,'sample');assert.match(result.markdown??'',/Structured/);assert.deepEqual(result.warnings,[]);await rm(dir,{recursive:true,force:true});
});

test('does not pretend to parse binary documents without Docling',async()=>{
 const dir=await mkdtemp(path.join(tmpdir(),'munin-doc-bin-'));const source=path.join(dir,'sample.pdf');const out=path.join(dir,'out');await writeFile(source,'fake','utf8');const execFileImpl:any=async()=>{throw new Error('not found')};const result=await ingestDocument(source,out,{execFileImpl});assert.equal(result.engine,'native-fallback');assert.equal(result.markdown,undefined);assert.match(result.warnings.join(' '),/requer Docling/i);await rm(dir,{recursive:true,force:true});
});
