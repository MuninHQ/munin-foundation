...
class SuccessfulRuntime implements GoalRuntime {
  ...
  async run(planId: string): Promise<ExecutionPlan> {
    const plan = this.plans.get(planId)!; plan.status = 'DONE'; plan.updatedAt = new Date().toISOString(); plan.tasks[0].status = 'DONE'; plan.tasks[0].result = 'Validated local autonomous result'; return plan;
  }
}
