...
test('strong adopted candidate is promoted into canonical project memory',async()=>{
 const dir=await mkdtemp(path.join(os.tmpdir(),'munin-radar-promotion-'));try{
  ...
  const fetcher=async()=>new Response(JSON.stringify({items:[{full_name:'example/strong-tool',html_url:'https://github.com/example/strong-tool',description:'strong',archived:false,stargazers_count:10000,forks_count:1000,open_issues_count:2,created_at:new Date(Date.now()-100*86_400_000).toISOString(),pushed_at:new Date().toISOString(),updated_at:new Date().toISOString(),license:{spdx_id:'MIT'}}]}),{status:200});
...
test('adopt assessment does not promote when benchmark holds',async()=>{
 const dir=await mkdtemp(path.join(os.tmpdir(),'munin-radar-hold-'));try{
  const log=new JsonCapabilityDecisionLog(path.join(dir,'decisions.json'));const memory=new ProjectMemoryStore(path.join(dir,'project-memory.json'));
  const fetcher=async()=>new Response(JSON.stringify({items:[{full_name:'example/held-tool',html_url:'https://github.com/example/held-tool',description:'held',archived:false,stargazers_count:10000,forks_count:1000,open_issues_count:2,created_at:new Date(Date.now()-100*86_400_000).toISOString(),pushed_at:new Date().toISOString(),updated_at:new Date().toISOString(),license:{spdx_id:'MIT'}}]}),{status:200});
