...
test('writes append-only audit records',async()=>{
 const dir=await mkdtemp(path.join(tmpdir(),'munin-policy-'));const file=path.join(dir,'audit.jsonl');const audit=new ActionAuditLog(file);const result=evaluateAction({class:'read',tool:'read-file',target:'README.md'});await audit.append(result);await audit.append(result);const lines=(await readFile(file,'utf8')).trim().split('\n');assert.equal(lines.length,2);assert.equal(JSON.parse(lines[0]).decision,'allow');await rm(dir,{recursive:true,force:true});
});
