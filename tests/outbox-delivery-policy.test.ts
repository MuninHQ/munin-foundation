...
async function setup(root: string) {
  const lease = await new PersistentLeaseStore(root).acquire('plan:1', 'worker-a', 60_000, 1_000);
...
test('delays retries using exponential backoff', async () => {
  ...
  try {
    ...
    await outbox.enqueue(adapter.id, await setup(root));
...
test('moves exhausted entries to dead-letter', async () => {
  ...
  try {
    ...
    const summary = await outbox.dispatch([adapter], { now: 1_010 });
    const [entry] = await outbox.list();
    assert.equal(summary.deadLettered, 1);
    assert.equal(entry.status, 'dead-letter');
    assert.equal(entry.deadLetteredAt, new Date(1_010).toISOString());
    assert.equal((await outbox.dispatch([adapter], { now: 2_000 })).attempted, 0);
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test('supports explicit dead-letter requeue', async () => {
  ...
  try {
    ...
    const adapter = new FailingAdapter();
    const queued = await outbox.enqueue(adapter.id, await setup(root));
    await outbox.dispatch([adapter], { now: 1_000 });
    const requeued = await outbox.requeueDeadLetter(queued.id, 2_000);
