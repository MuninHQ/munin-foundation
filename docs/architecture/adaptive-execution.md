...
Feedback is a replaceable, validated record on one outcome. Schema-v1 `adaptive-outcomes.json` state loads without data loss; the next successful save or feedback mutation persists schema version 2. Unsupported or malformed state fails closed without rewriting the file, and version-1 records ignore any injected feedback. Mutations use the existing atomic JSON writer and are serialized by canonical outcome-file path within the process. A rating-only pending-audit item is committed atomically with feedback, then delivered idempotently as `adaptive.outcome.feedback.updated`; a temporary event-log failure leaves it pending for a later store operation and does not report an already committed feedback update as failed. Neither the pending item nor the event contains the feedback reason.
