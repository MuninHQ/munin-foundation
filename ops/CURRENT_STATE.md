...
- The GitHub-inspired operator UX set is complete: a global mobile command palette, a read-only visual flow viewer over real orchestration traces, and a full operational chat surface that separates deterministic local commands from explicit sanitized ChatGPT handoff.
- The Proactive Operator Hub now derives Today, Waiting For, approval receipts, people signals, browser approval plans, agent analytics and morning/evening briefs from existing local state. Calendar remains visibly read-only and disconnected until explicit OAuth authorization.
