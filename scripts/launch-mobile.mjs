...
async function main(){console.log('[Munin Mobile] Building runtime and mobile web…');if(!skipBuild)await runDone(NPM_COMMAND,['run','build:core'],'core build');await ensureApi();monitorApi();if(!skipWeb){if(!(await portOpen(WEB_PORT)))run(NPM_COMMAND,['run','web','--','--host','0.0.0.0','--port',String(WEB_PORT),'--strictPort'],'mobile web');if(!(await waitFor(WEB_PORT)))throw new Error('Munin web did not start.');}console.log('\n=== MUNIN MOBILE READY ===');console.log(`Token: ${token}`);console.log('Salve este token no primeiro acesso; ele permanece estável neste PC.');if(!skipTailscale&&tailscaleAvailable()){const served=configureServe();const dns=tailscaleDns();if(served&&dns)console.log(`iPhone (Tailscale HTTPS): https://${dns}/mobile.html`);else console.log('Tailscale está instalado, mas Serve requer configuração/consentimento. Rode: tailscale serve --bg '+WEB_PORT);}if(!skipWeb){for(const address of localAddresses())console.log(`LAN: http://${address}:${WEB_PORT}/mobile.html`);console.log('Acesso via Tailscale Serve é preferível: privado no tailnet e HTTPS.');}console.log('[Munin Mobile] Guardian active. Keep this window open.');await new Promise(()=>{});}
function shutdown(){if(apiMonitor)clearInterval(apiMonitor);for(const child of children)if(!child.killed)child.kill('SIGTERM');process.exit(0)}
