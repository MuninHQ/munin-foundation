...
async function ensureWeb(){if(await portOpen(WEB_PORT)){console.log('[Munin Mobile] Replacing existing web runtime with the supervised production server…');if(!killPort(WEB_PORT))throw new Error(`Port ${WEB_PORT} is occupied by another web runtime. Stop that process and retry.`);await new Promise(r=>setTimeout(r,700));if(await portOpen(WEB_PORT))throw new Error(`Port ${WEB_PORT} is still occupied after web restart attempt.`);}const child=run(process.execPath,[WEB_SERVER_ENTRY],'mobile web');console.log(`[Munin Mobile] Web started (PID ${child.pid}).`);if(!(await waitFor(WEB_PORT)))throw new Error('Munin web did not start.');if(!(await webRouteHealthy()))throw new Error('Munin web started but mobile.html is unavailable or stale.');}
