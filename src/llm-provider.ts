...
export type LlmProviderStatus={enabled:boolean;provider:'openai-compatible'|'disabled';model?:string;baseUrl?:string;source?:'settings'|'environment'};
type Normalized={command?:string;reply?:string;confidence?:number};

type ProviderConfig={baseUrl:string;apiKey:string;model:string;source:'settings'|'environment'};

async function providerConfig():Promise<ProviderConfig|undefined>{
  const settings=await loadLlmSettings();
  if(settings.enabled&&settings.baseUrl&&settings.apiKey&&settings.model)return {baseUrl:settings.baseUrl,apiKey:settings.apiKey,model:settings.model,source:'settings'};
  ...
  if(baseUrl&&apiKey&&model)return {baseUrl,apiKey,model,source:'environment'};
  return undefined;
}
...
export async function llmProviderStatus():Promise<LlmProviderStatus>{
  ...
  return {enabled:true,provider:'openai-compatible',model:config.model,baseUrl:config.baseUrl,source:config.source};
}
...
export async function testLlmProvider():Promise<{ok:true;model:string;source:string;message:string}>{
  const config=await providerConfig();
  if(!config)throw new Error('Nenhum provider LLM ativo.');
  const payload=await callProvider(config,[{role:'system',content:'Responda somente com OK.'},{role:'user',content:'Teste de conexão do Munin.'}]);
  const content=extractContent(payload);
  if(!content)throw new Error('Provider respondeu sem conteúdo.');
  return {ok:true,model:config.model,source:config.source,message:content.trim().slice(0,120)};
}
...
export async function normalizeWithLlm(userCommand:string):Promise<Normalized|undefined>{
  ...
  const payload=await callProvider(config,[{role:'system',content:system},{role:'user',content:`Contexto Munin:\n${JSON.stringify(context)}\n\nPedido:\n${userCommand}`}]);
  const content=extractContent(payload); if(!content)return undefined; return parseJson(content);
}
