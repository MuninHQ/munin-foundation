...
export type ImageSettings={enabled:boolean;provider:'openai';baseUrl:string;apiKey:string;model:string;size:'1024x1536'|'1024x1024';quality:'low'|'medium'|'high';updatedAt:string};
...
const empty=():ImageSettings=>({enabled:false,provider:'openai',baseUrl:'https://api.openai.com/v1',apiKey:'',model:'gpt-image-1',size:'1024x1536',quality:'medium',updatedAt:new Date(0).toISOString()});
...
export async function saveImageSettings(input:Partial<ImageSettings>):Promise<PublicImageSettings>{const current=await loadImageSettings();const next:ImageSettings={...current,...input,provider:'openai',apiKey:input.apiKey===undefined||input.apiKey===''?current.apiKey:input.apiKey.trim(),baseUrl:(input.baseUrl??current.baseUrl).trim(),model:(input.model??current.model).trim(),updatedAt:new Date().toISOString()};if(next.enabled&&(!next.baseUrl||!next.apiKey||!next.model))throw new Error('Base URL, API key e modelo são obrigatórios para ativar geração de imagem.');try{new URL(next.baseUrl);}catch{throw new Error('Base URL inválida.');}await mkdir(dirname(file),{recursive:true});await writeFile(file,JSON.stringify(next,null,2),'utf8');return publicImageSettings(next);}
