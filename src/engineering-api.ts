...
export async function handleEngineeringApi(request:IncomingMessage,response:ServerResponse):Promise<void>{
 ...
 try{
  ...
  if(request.method==='POST'&&url.pathname==='/api/mobile/engineering'){
   const body=await readJsonBody(request,100_000);const objective=await objectiveFromBody(body);const result=await new EngineeringAgentRuntime().execute(objective);return json(request,response,result.status==='failed'?422:200,result);
  }
