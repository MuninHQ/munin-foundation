...
function verificationUrlFromBody(body:Record<string,unknown>){if(body.verificationUrl===undefined)return undefined;if(typeof body.verificationUrl!=='string')throw new Error('verificationUrl must be a string.');return validateBrowserInspectionUrl(body.verificationUrl)}
...
export async function handleEngineeringApi(request:IncomingMessage,response:ServerResponse):Promise<void>{
 ...
 try{
  if(request.method==='POST'&&url.pathname==='/api/mobile/engineering/jobs'){
   const body=await readJsonBody(request,100_000);const objective=await objectiveFromBody(body);const verificationUrl=verificationUrlFromBody(body);return json(request,response,202,jobs.start(objective,verificationUrl));
  }
  ...
  if(request.method==='POST'&&url.pathname==='/api/mobile/engineering'){
   const body=await readJsonBody(request,100_000);const objective=await objectiveFromBody(body);const verificationUrl=verificationUrlFromBody(body);const readiness=await prepareEngineeringProvider();if(!readiness.ok)return json(request,response,422,unavailable(objective,readiness.message,readiness.evidence));const result=await runEngineeringMission(objective,process.cwd(),undefined,{verificationUrl});result.events.unshift({phase:'inspect',message:readiness.message,at:new Date().toISOString(),evidence:readiness.evidence});return json(request,response,result.status==='failed'?422:result.status==='needs_user'?409:200,result);
  }
