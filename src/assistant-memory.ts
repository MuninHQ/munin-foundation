...
const file=join(process.cwd(),'data','runtime','assistant-memory.json');
...
export async function saveAssistantMemory(memory:AssistantMemory){memory.turns=memory.turns.slice(-30);memory.updatedAt=new Date().toISOString();await mkdir(dirname(file),{recursive:true});await writeFile(file,JSON.stringify(memory,null,2),'utf8')}
