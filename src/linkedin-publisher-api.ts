...
import { approveForPublication, markManuallyPublished, publicationPackage, publisherBrandPreflight, publisherPolicy, publisherQueue, revokePublicationApproval, schedulePublication } from './linkedin-publisher.js';
...
export async function handleLinkedInPublisher(request:IncomingMessage,response:ServerResponse):Promise<void>{
 ...
 try{
  ...
  const schedule=url.pathname.match(/^\/api\/linkedin-publisher\/([^/]+)\/schedule$/);if(request.method==='POST'&&schedule){const input=await readJsonBody(request,100_000);return json(request,response,200,await schedulePublication(schedule[1],{scheduledFor:requireText(input.scheduledFor,'scheduledFor'),confirmation:requireText(input.confirmation,'confirmation'),note:typeof input.note==='string'?input.note:undefined}));}
