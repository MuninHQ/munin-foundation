...
export class ContextStore {
  ...
  async event(type: string, entityType: MuninEvent['entityType'], entityId: string, payload: Record<string, unknown> = {}): Promise<MuninEvent> { await this.ensure(); const event: MuninEvent = { id: randomUUID(), type, entityType, entityId, timestamp: new Date().toISOString(), payload }; await appendFile(this.eventsPath(), JSON.stringify(event) + '\n', 'utf8'); return event; }
