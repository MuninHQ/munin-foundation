...
export async function handleVisualAssets(request:IncomingMessage,response:ServerResponse):Promise<void>{if(request.method==='OPTIONS')return json(response,204,{});const url=new URL(request.url??'/','http://127.0.0.1');try{
  if(request.method==='GET'&&url.pathname==='/api/visual-assets/health')return json(response,200,{status:'ok',service:'munin-visual-assets',visualIntelligence:await visualIntelligenceStatus()});
  if(request.method==='GET'&&url.pathname==='/api/visual-assets'){const state=await loadLinkedInContent();const records=await visualIntelligenceRecords();return json(response,200,{assets:state.visualReferences.map(asset=>({...asset,intelligence:records[asset.id]})),posts:state.posts.map(p=>({id:p.id,title:p.title,status:p.status,publishedAt:p.publishedAt,visualReferenceId:p.visualReferenceId})),visualIntelligence:await visualIntelligenceStatus()});}
  ...
  if(request.method==='POST'&&url.pathname==='/api/visual-assets/analyze-all'){const input=await body(request);const limit=typeof input.limit==='number'?input.limit:20;return json(response,200,await analyzeAllVisualReferences(limit));}
  const analyze=url.pathname.match(/^\/api\/visual-assets\/([^/]+)\/analyze$/);if(request.method==='POST'&&analyze)return json(response,200,await analyzeVisualReference(analyze[1]));
