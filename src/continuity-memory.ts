...
export class ContinuityMemoryStore {
  ...
  async list(includeStale=true){const records=await this.loadAll();return records.filter(record=>includeStale||calculatedFreshness(record)!=='stale').map(record=>({...record,freshness:calculatedFreshness(record)})).sort((a,b)=>timestamp(b)-timestamp(a));}
  async get(id:string){const record=(await this.loadAll()).find(item=>item.id===id);return record?{...record,freshness:calculatedFreshness(record)}:undefined;}
  async correct(id:string,patch:Pick<MemoryInput,'subject'|'content'|'tags'> & Partial<Pick<MemoryInput,'confidence'|'lastConfirmedAt'>>){const records=await this.loadAll();const record=records.find(item=>item.id===id);if(!record)throw new Error('Memory record not found.');record.subject=patch.subject.trim();record.content=patch.content.trim();record.tags=[...new Set(patch.tags.map(tag=>tag.trim()).filter(Boolean))];if(patch.confidence)record.confidence=patch.confidence;record.lastConfirmedAt=patch.lastConfirmedAt??new Date().toISOString();record.freshness='current';record.source=`${record.source}:user-corrected`;await this.saveAll(records);return {...record};}
  async remove(id:string){const records=await this.loadAll();const next=records.filter(item=>item.id!==id);if(next.length===records.length)return {removed:false,total:records.length};await this.saveAll(next);return {removed:true,total:next.length};}
  async clear(){const backup=await this.backup();await this.saveAll([]);return {cleared:true,backup};}
  async export(){return {exportedAt:new Date().toISOString(),records:await this.list(true)};}
