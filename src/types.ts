...
export type EntityType = 'project' | 'decision' | 'action' | 'job' | 'research';
...
export interface JobOpportunity {
  id: string; company: string; role: string; source?: string; link?: string; status: JobStatus;
  fitScore: number; matchedSignals: string[]; salaryRange?: string; currency?: string;
  recruiter?: string; hiringManager?: string; nextAction?: string; followUpAt?: string;
  appliedAt?: string; lastContactAt?: string; closedReason?: string; notes?: string;
...
export interface ResearchEvidence {
  id: string; title: string; url: string; sourceType: 'primary' | 'secondary';
  note?: string; capturedAt: string;
}

export interface ResearchSynthesis {
  version: number; summary: string; createdAt: string; evidenceIds: string[];
}

export interface ResearchRecord {
  id: string; question: string; status: ResearchStatus; projectId?: string;
  evidence: ResearchEvidence[]; syntheses: ResearchSynthesis[];
  createdAt: string; updatedAt: string;
}
...
export interface MuninState { projects: Project[]; decisions: Decision[]; actions: Action[]; jobs: JobOpportunity[]; research: ResearchRecord[]; relations: ContextRelation[]; }
