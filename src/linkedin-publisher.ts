...
export async function markManuallyPublished(postId:string,input:{url:string;confirmation:string;note?:string}):Promise<PublisherRecord>{await assertPost(postId);if(input.confirmation!=='I_PUBLISHED_THIS_MANUALLY')throw new Error('Manual publication confirmation is required');let parsed:URL;try{parsed=new URL(input.url);}catch{throw new Error('A valid published URL is required');}if(!/^https?:$/.test(parsed.protocol))throw new Error('Published URL must use http or https');const state=await loadState();const record=recordFor(state,postId);if(record.status!=='approved')throw new Error('Post must be approved before it can be marked published');const publishedAt=now();record.status='published';record.publishedAt=publishedAt;record.publishedUrl=parsed.toString();record.note=input.note?.trim()||record.note;record.updatedAt=publishedAt;await saveState(state);await syncEditorialHistory(postId,publishedAt,record.publishedUrl);return record;}
