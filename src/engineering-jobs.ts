...
export class EngineeringJobManager{
 ...
 private async run(id:string){const job=this.jobs.get(id);if(!job)return;job.status='running';job.startedAt=new Date().toISOString();this.persist();try{if(!existsSync(path.join(this.repo,'.git'))){const result=await runEngineeringMission(job.objective,this.repo);job.result=result;job.status=result.status;job.completedAt=new Date().toISOString();return}const readiness=await prepareEngineeringProvider();if(!readiness.ok){const result=providerUnavailableResult(job.objective,readiness.message,readiness.evidence);job.result=result;job.status=result.status;job.completedAt=new Date().toISOString();return}const result=await runEngineeringMission(job.objective,this.repo);result.events.unshift({phase:'inspect',message:readiness.message,at:new Date().toISOString(),evidence:readiness.evidence});job.result=result;job.status=result.status;job.completedAt=new Date().toISOString()}catch(error){job.status='failed';job.error=error instanceof Error?error.message:String(error);job.completedAt=new Date().toISOString()}finally{this.persist()}}
}
