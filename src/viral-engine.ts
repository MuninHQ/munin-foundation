...
import { assessViralProductionReadiness } from './viral-production-readiness.js';
export { assessViralProductionReadiness, type ViralProductionReadiness } from './viral-production-readiness.js';
...
export class ViralEngineStore {
  ...
  async snapshot() {
    ...
    const executiveQueue = ranked.filter(topic => !['published','measured'].includes(topic.stage)).slice(0, 20).map(topic => ({ title: topic.title, score: topic.opportunityScore, stage: topic.stage, ...assessViralProductionReadiness(topic, state.signals.find(signal => signal.id === topic.signalId), ranked) })).sort((a, b) => Number(b.state === 'ready') - Number(a.state === 'ready') || b.score - a.score);
