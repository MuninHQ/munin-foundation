...
async function countChunks(file?:string){if(!file)return 0;try{const raw=await fs.readFile(file,'utf8');return raw.split(/\r?\n/).filter(Boolean).length}catch{return 0}}
...
export async function benchmarkDocuments(inputDir:string,outputDir=path.resolve('data/runtime/document-benchmark')):Promise<DocumentBenchmarkReport>{
 ...
 for(const file of files){const stat=await fs.stat(file);const artifactOut=path.join(out,path.basename(file).replace(/[^a-z0-9._-]+/gi,'_'));const t0=performance.now();
  try{const result=await ingestDocument(file,artifactOut);const chunks=await countChunks(result.chunksPath);items.push({file:path.basename(file),extension:path.extname(file).toLowerCase(),bytes:stat.size,engine:result.engine,durationMs:Math.round(performance.now()-t0),chunks,warnings:result.warnings,ok:Boolean(result.markdown||result.json)});}
