...
export class OAuthReconnectRequiredError extends Error {
 constructor(public readonly provider:OAuthProvider, message=`${provider} connection must be renewed`){super(message);this.name='OAuthReconnectRequiredError'}
}
...
const root=()=>dataDir(); const file=()=>path.join(root(),'oauth.json');
...
export async function accessToken(provider:OAuthProvider):Promise<string>{assertReadOnlyOAuthScopes(provider);const state=await load(),token=state.tokens[provider];if(!token)throw new OAuthReconnectRequiredError(provider,`${provider} is not connected`);if(token.expiresAt>Date.now()+60000)return token.accessToken;if(!token.refreshToken){delete state.tokens[provider];await save(state);throw new OAuthReconnectRequiredError(provider,`${provider} token expired; reconnect account`)}const cfg=config(provider),form=new URLSearchParams({client_id:cfg.clientId!,refresh_token:token.refreshToken,grant_type:'refresh_token'});if(cfg.clientSecret)form.set('client_secret',cfg.clientSecret);if(provider==='outlook')form.set('scope',cfg.scope);const response=await fetch(cfg.token,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:form}),data=await response.json() as any;if(!response.ok){const detail=String(data.error_description??data.error??'OAuth refresh failed');if(data.error==='invalid_grant'||/expired|revoked|invalid grant/i.test(detail)){delete state.tokens[provider];await save(state);throw new OAuthReconnectRequiredError(provider,`${provider} authorization expired or was revoked`)}throw new Error(detail)}state.tokens[provider]={...token,accessToken:data.access_token,refreshToken:data.refresh_token??token.refreshToken,expiresAt:Date.now()+Number(data.expires_in??3600)*1000,scope:data.scope??token.scope};await save(state);return data.access_token}
