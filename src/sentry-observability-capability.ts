import { listSentryIssues, sentryConfigFromEnv, type SentryListIssuesOptions } from './sentry-readonly-client.js';
...
export type SentryObservabilityCapabilityInput=
 ...
 | {action:'list-issues';options?:SentryListIssuesOptions};
...
export type SentryObservabilityCapabilityOutput=
 ...
 | {action:'list-issues';issues:SentryIssueSnapshot[];policy:ReturnType<typeof sentryObservabilityPolicy>};
...
export function createSentryObservabilityCapability():RuntimeCapability<SentryObservabilityCapabilityInput,SentryObservabilityCapabilityOutput>{return {
 ...
 async execute(input){
  ...
  if(input.action==='list-issues')return {action:'list-issues',issues:await listSentryIssues(sentryConfigFromEnv(),input.options),policy:sentryObservabilityPolicy()};
